...
This module provides BusinessInfinity-specific configuration that extends
the generic runtime configuration.

BusinessInfinity is now primarily configuration-driven, with most of the
infrastructure handled by the generic runtime and AgentOperatingSystem.
"""
...
class BusinessInfinityConfig:
    ...
    def from_env(cls) -> 'BusinessInfinityConfig':
        ...
        return cls(
            ...
            autonomous_boardroom=os.getenv('AUTONOMOUS_BOARDROOM', 'true').lower() == 'true',
            covenant_enabled=os.getenv('COVENANT_ENABLED', 'true').lower() == 'true',
            linkedin_verification_enabled=os.getenv('LINKEDIN_VERIFICATION_ENABLED', 'true').lower() == 'true',
            workflows_enabled=os.getenv('WORKFLOWS_ENABLED', 'true').lower() == 'true',
            analytics_enabled=os.getenv('ANALYTICS_ENABLED', 'true').lower() == 'true',
            mentor_mode_enabled=os.getenv('MENTOR_MODE_ENABLED', 'true').lower() == 'true',
            onboarding_enabled=os.getenv('ONBOARDING_ENABLED', 'true').lower() == 'true',
            network_discovery_enabled=os.getenv('NETWORK_DISCOVERY_ENABLED', 'true').lower() == 'true',
    ...
    def from_json(cls, config_path: str) -> 'BusinessInfinityConfig':
        ...
        runtime_config = RuntimeConfig.from_dict(runtime_config_data) if runtime_config_data else RuntimeConfig()
        
        # Create BI config
        bi_config = cls(**{k: v for k, v in config_data.items() if k in cls.__dataclass_fields__})
...
def load_bi_config(
    ...
    """
    Load BusinessInfinity configuration.
    
    Priority order:
    1. config_path (if provided)
    2. src/config/businessInfinity.json (if exists)
    ...
    # Try default BI config location
    default_path = "src/config/businessInfinity.json"
    if os.path.exists(default_path):
        return BusinessInfinityConfig.from_json(default_path)
...
def create_bi_config() -> BusinessInfinityConfig:
    """Create BusinessInfinity configuration with sensible defaults."""
    return load_bi_config()
