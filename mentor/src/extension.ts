...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('boardroom.listAgents', async () => {
      try {
        ...
        const agentList = agents.map(agent => 
          `• ${agent.name} (${agent.id}) - Version: ${agent.loraVersion}`
        ).join('\n');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('boardroom.chatWithAgent', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('boardroom.compareVersions', async () => {
      const agentId = await vscode.window.showInputBox({ 
...
`,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('boardroom.manage', async () => {
      const options = [
        'List Agents',
        'Chat with Agent', 
        'Start Fine-Tuning',
