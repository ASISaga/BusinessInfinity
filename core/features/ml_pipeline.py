"""
Consolidated ML Pipeline Management
Merges functionality from /ml_pipeline/manager.py
"""
...
# Import AML endpoints configuration
try:
    from ...ml_pipeline.endpoints import AML_ENDPOINTS
except ImportError:
...
class UnifiedMLManager:
    ...
    def __init__(self):
        ...
        self.subscription_id = env.get_optional("AZURESUBSCRIPTION_ID")
        self.resource_group_name = env.get_optional("AZURERESOURCEGROUP")
        self.workspace_name = env.get_optional("AZUREML_WORKSPACE")
        self.pipeline_name = env.get_optional("PIPELINEENDPOINT_NAME")
    ...
    def get_client(self):
        ...
        if not all([self.subscription_id, self.resource_group_name, self.workspace_name]):
            raise EnvironmentError("Missing required Azure ML configuration")
